...
import { Button, Col, Flex, Image, Row, Select } from "antd";
...
const HomeView: React.FC = () => {
	...
	const createUrlencoded = () => {
		...
		data.append("email", "2394630102@qq.com");
	...
	const createFormData = () => {
		...
		data.append("email", "2394630102@qq.com");
		data.append("password", "7777777");
		PostApi.fetchExpressPostFormDataApi(data).then(res => {
			console.log(res);
		});
	};
	const [imageSrc, setImageSrc] = useState<string>("");
	const createFormDataFile = () => {
		const el = document.getElementById("avatar") as HTMLInputElement;
		if (el.files?.length) {
			const data = new FormData();
			data.append("avatar", el?.files[0]);
			PostApi.fetchExpressPostFormDataFileApi(data).then(blob => {
				// 创建一个 URL 对象
	...
	return (
		<Flex gap="small" wrap>
			...
			<Button type="primary" onClick={createFormData}>
				multipart/form-data
			</Button>
			<Row>
				<Col>
				...
				<Col>
					<Button type="primary" onClick={createFormDataFile}>
						send
					</Button>
				</Col>
				<Col>
